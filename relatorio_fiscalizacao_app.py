...
def gerar_pdf(dados):
    ...
    pdf.cell(200, 10, "RELATÓRIO DE FISCALIZAÇÃO - CONTRATO 300000219/2022", ln=True, align="C")
    pdf.ln(10)
    pdf.multi_cell(0, 10, f"Fiscal: {dados['fiscal']}\nData da Fiscalização: {dados['data']}\nMês de Referência: {dados['mes']}\nUnidade: {dados['unidade']}\nMunicípio: {dados['municipio']}\n\nOcorrências:\n{dados['ocorrencias']}\n\nConformidades:\n{dados['conformidades']}\n\nMonitoramento Eletrônico:\nTipo de Kit: {dados['kit']}\nStatus: {dados['status']}\nObservações: {dados['obs_kit']}\n\nRecomendações:\n{dados['recomendacoes']}")

    if dados['nomes_fotos']:
        pdf.add_page()
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(200, 10, "Fotos da Fiscalização:", ln=True)
        for i, nome in enumerate(dados['nomes_fotos']):
            ...
            pdf.cell(85, 5, f"Foto {i+1}", align="C")
...
if submitted:
    ...
    if os.path.exists(pdf_path):
        with open(pdf_path, "rb") as file:
            st.download_button("📄 Baixar Relatório em PDF", data=file.read(), file_name=os.path.basename(pdf_path), mime="application/pdf")
    else:
